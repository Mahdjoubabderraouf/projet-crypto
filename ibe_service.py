...
class IBEService:
    ...
    def serialize_key(self, key_dict):
        ...
        return {
            'd0': base64.b64encode(objectToBytes(key_dict['d0'], self.group)).decode('utf-8'),
            'd1': base64.b64encode(objectToBytes(key_dict['d1'], self.group)).decode('utf-8')
        }

    def deserialize_key(self, key_dict):
        """Convert serialized key back to crypto objects"""
        return {
            'd0': bytesToObject(base64.b64decode(key_dict['d0'].encode('utf-8')), self.group),
            'd1': bytesToObject(base64.b64decode(key_dict['d1'].encode('utf-8')), self.group)
        }
    
    def extract_key(self, user_id: str):
        """Generate private key for user"""
        if not self.master_key:
            raise ValueError("IBE not initialized")
        
        r = self.group.random(ZR)
        h_id = self.group.hash(user_id, ZR)
        U_path = (self.params['U'][0][0] * 
                 self.params['U'][1][0] * 
                 self.params['U'][2][0])
        
        return {
            'd0': self.master_key['g2_alpha'] * (U_path ** r),
            'd1': self.params['g'] ** r
        }
    

    def encrypt(self, ID, params, M):
        """
        ...
        return {'A':A, 'B':B, 'C':C }
    def decrypt(self, dID, cipher_text):
        """
